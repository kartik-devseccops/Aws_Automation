...
# Function to fetch the previous day's cost for percentage calculation
def fetch_previous_day_cost(service_name):
    client = boto3.client("ce", region_name=boto3.session.Session().region_name)
    today = datetime.today().strftime('%Y-%m-%d')
    ...
    try:
        response = client.get_cost_and_usage(
            ...
            Granularity="DAILY",
            Metrics=["UnblendedCost"],
            Filter={"Dimensions": {"Key": "SERVICE", "Values": [aws_service_name]}}
        )

        if response['ResultsByTime'] and response['ResultsByTime'][0]['Total']:
            return float(response['ResultsByTime'][0]['Total']['UnblendedCost']['Amount'])
        return 0.0

    except Exception as e:
        logger.error(f"Error fetching previous day cost for {service_name}: {e}")
        return 0.0

# Updated fetch_budget_data() to calculate percentage change
    ...
    try:
        ...
        while True:
            ...
            for budget in response.get('Budgets', []):
                ...
                previous_day_cost = fetch_previous_day_cost(budget["BudgetName"])
                percentage_change = (
                    round(((total_spent - previous_day_cost) / previous_day_cost) * 100, 2)
...
# Updated Lambda Handler - Now Shows Percentage Change
def lambda_handler(event, context):
    ...
    for budget in budget_data:
        ...
        percentage_change = budget["percentage_change"]
        ...
        report_lines.append(
            ...
            f"({change_icon} {abs(percentage_change)}%)\n"
